...
public class ContractDescriptorBuilder : MockMemorySpace.Builder
{
    ...
    public ContractDescriptorTarget CreateTargetFromRawDescriptor(byte[] descriptor, byte[] descriptorJson, byte[] pointerData)
    {
        ...
        MockMemorySpace.MemoryContext memoryContext = GetMemoryContext();
        return ContractDescriptorTarget.Create(ContractDescriptorAddr, memoryContext.ReadFromTarget, memoryContext.WriteToTarget, (_, _, _) => throw new NotImplementedException("Tests do not provide GetTargetThreadContext"), (ulong _, out ulong _) => throw new NotImplementedException("Tests do not provide AllocVirtual"), [Contracts.CoreCLRContracts.Register]);
    }
}
